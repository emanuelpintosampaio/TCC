...
# Raio
ax1.plot(tempo, raio_euler * 1e6, '^-', color='#0D00FF',lw=2, ms=4,
         label=f'r_final = {raio_euler[-1] * 1e6} µm')
...
# Temperatura
ax2.plot(tempo, temperatura_euler - 273.15,'^-', color='#0D00FF',lw=2, ms=4,
        label=f'T_final = {temperatura_euler[-1] - 273.15} °C')
...
# Massa
ax3.semilogx(tempo, massa_euler, '^-', color='#0D00FF',lw=2, ms=4,
            label=f'Massa final: {massa_final_euler} mol')
